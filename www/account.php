...
account_header($title)
{
  ...
  $links = array("Change Password" => "$PHP_SELF?P");
  if ($LOGIN_LEVEL == AUTH_ADMIN)
    $links["Manage Accounts"] = "$PHP_SELF?A";
  if ($LOGIN_LEVEL > AUTH_USER)
    $links["New/Pending"] = "$PHP_SELF?L";
  $links["Logout"] = "$PHP_SELF?X";
...
switch ($op)
{
  case 'A' :
	...
	$result = db_query("SELECT * FROM user ORDER BY name");
        ...
        html_start_table(array("Username", "EMail", "Level"), "80%");
