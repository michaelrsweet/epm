...
class user
{
  ...
  {
    global $AUTH_LEVELS, $AUTH_MASKS, $AUTH_PROJECTS,
    ...
    // projects
    if ($LOGIN_LEVEL == AUTH_ADMIN || $this->projects)
    {
      if ($this->projects_valid)
	$hclass = "valid";
      else
	$hclass = "invalid";
      print("<tr><th class='$hclass' align='right' valign='top' nowrap>Projects:</th><td>");
	...
	if ($LOGIN_LEVEL == AUTH_ADMIN)
	{
          $form = "project_" . strtolower(str_replace(" ", "_", $name));
  ...
  loadform()
  {
    global $_GET, $_POST, $AUTH_MASKS, $AUTH_PROJECTS, $LOGIN_LEVEL;
    ...
    if ($LOGIN_LEVEL == AUTH_ADMIN)
    {
      ...
      while (list($link_id, $name) = each($AUTH_PROJECTS))
      {
        ...
        if (array_key_exists($form, $_POST))
          $this->projects |= $AUTH_MASKS[$link_id];
      }
    }
  ...
  save()
  {
	...
	$dprev = db_escape($this->previous_name);
        ...
        db_query("UPDATE ppd SET create_user = '$dname' "
	        ."WHERE create_user = '$dprev'");

        db_query("UPDATE ppd SET modify_user = '$dname' "
	        ."WHERE modify_user = '$dprev'");
        ...
        db_query("UPDATE strsubsystems SET create_user = '$dname' "
	        ."WHERE create_user = '$dprev'");

        db_query("UPDATE strsubsystems SET modify_user = '$dname' "
	        ."WHERE modify_user = '$dprev'");

        db_query("UPDATE strtext SET create_user = '$dname' "
	        ."WHERE create_user = '$dprev'");

        db_query("UPDATE strversions SET create_user = '$dname' "
	        ."WHERE create_user = '$dprev'");

        db_query("UPDATE strversions SET modify_user = '$dname' "
	        ."WHERE modify_user = '$dprev'");
  ...
  validate()
  {
    ...
    if ($this->projects > 63 || $this->projects < 0)
    {
      $this->projects_valid = FALSE;
      $valid = FALSE;
    }
    else
      $this->projects_valid = TRUE;
  ...
  view()
  {
    ...
    if ($this->projects)
    {
      print("<tr><th align='right' valign='top' nowrap>Projects:</th><td>");
      reset($AUTH_PROJECTS);
      while (list($link_id, $name) = each($AUTH_PROJECTS))
	if ($this->projects & $AUTH_MASKS[$link_id])
	  print("$name<br />");
